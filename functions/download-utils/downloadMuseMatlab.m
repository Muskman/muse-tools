%% Function for downloading human readable format data from MuSe Dataset repository
% usage:
...
function [] = downloadMuseMatlab(destination_folder, robot_run, sync_type)

    curr_folder = pwd;
    server = 'http://103.246.106.250/data/full-data/matlab-format/';

    % check if destination is valid
    try
        cd(destination_folder)
        create_dir('muse/full-data/matlab-format')
        cd muse/full-data/matlab-format
    catch
...
function [] = download_subdata(srv, data_str, num_chunks)
    for j = 0:num_chunks
        ...
        fprintf(strcat('Downloading matlab-format ',data_str(1:8), ' chunk ',num2str(j,'%03.f'),'\r'))
        fprintf('...\r')
