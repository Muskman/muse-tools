...
function [] = downloadMuseHumanReadable(destination_folder, robot_run, sync_type)

    curr_folder = pwd;
    server = 'http://103.246.106.250/data/full-data/human-readable-format/';

    % check if destination is valid
    try
        cd(destination_folder)
        create_dir('muse/full-data/human-readable-format')
        cd muse/full-data/human-readable-format
    catch
